...
class _LoginScreenState extends State<LoginScreen> {
  ...
  Future<void> _loginKader() async {
    ...
    if (mounted) {
      if (ok) context.go('/dashboard/kader');
      else _showError(auth.errorMessage ?? 'Login gagal');
    }
  }
  ...
  Future<void> _loginBidan() async {
    ...
    if (mounted) {
      if (ok) context.go('/dashboard/bidan');
      else _showError(auth.errorMessage ?? 'Login gagal');
    }
  }
  ...
  Future<void> _loginOrtu() async {
    ...
    if (mounted) {
      if (ok) context.go('/dashboard/ortu');
      else _showError(auth.errorMessage ?? 'Login gagal');
    }
  }
          ...
          child: SingleChildScrollView(
            ...
            child: Column(
              ...
              children: [
                ...
                const Text('Sistem Informasi Posyandu Anak Digital',
                    style: TextStyle(fontSize: 13, color: Colors.white70),
                    textAlign: TextAlign.center),
                const SizedBox(height: 32),
                _selectedRole == -1 ? _buildPilihRole() : _buildFormLogin(),
              ],
            ),
          ),
        ),
  ...
  Widget _buildPilihRole() {
    return _card(
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        _roleButton(0, Icons.people_alt_rounded, 'Kader', 'Petugas posyandu', const Color(0xFF0D6EFD)),
        const SizedBox(height: 10),
        _roleButton(1, Icons.medical_services_rounded, 'Bidan', 'Tenaga kesehatan', const Color(0xFF198754)),
        const SizedBox(height: 10),
        _roleButton(2, Icons.family_restroom_rounded, 'Orang Tua', 'Wali balita', const Color(0xFFE67E22)),
      ]),
    );
  }

  Widget _roleButton(int idx, IconData icon, String label, String sub, Color color) {
      ...
      child: Container(
        ...
        child: Row(children: [
          ...
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            Text(sub, style: const TextStyle(fontSize: 12, color: _textGrey)),
          ])),
  ...
  Widget _buildFormLogin() {
    ...
    return _card(
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [
          ...
          Container(
            ...
            child: Row(mainAxisSize: MainAxisSize.min, children: [
              Icon(role['icon'] as IconData, size: 14, color: role['color'] as Color),
              const SizedBox(width: 6),
              Text(role['label'] as String,
                  style: TextStyle(fontSize: 13, fontWeight: FontWeight.w600, color: role['color'] as Color)),
            ]),
          ),
        ]),
        const SizedBox(height: 20),
        ...
        const SizedBox(height: 4),
        Text(_getSubtitle(), style: const TextStyle(fontSize: 13, color: _textGrey)),
        const SizedBox(height: 20),
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          auth.posyanduLoading
              ...
              : DropdownButtonFormField<PosyanduItem>(
                  ...
                  items: auth.posyanduList.map((p) => DropdownMenuItem(
                    value: p,
                    child: Text(p.namaPosyandu, style: const TextStyle(fontSize: 14)),
                  )).toList(),
                  ...
                  isExpanded: true,
                  // Buka dropdown ke atas agar tidak menutupi password
                  menuMaxHeight: 200,
                ),
          const SizedBox(height: 16),
          const Text('Password',
      ...
      builder: (_, auth, __) => Form(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          TextFormField(
            ...
            decoration: _inputDeco(hint: 'Masukkan 16 digit NIK anak').copyWith(counterText: ''),
          ),
          const SizedBox(height: 14),
          ...
          GestureDetector(
            ...
            child: Container(
              ...
              child: Row(children: [
                const Icon(Icons.calendar_month_rounded, size: 18, color: _textGrey),
                const SizedBox(width: 10),
                Text(_tglDisplay,
                    style: TextStyle(fontSize: 14,
                        color: _selectedDate == null ? _textGrey : _textDark)),
              ]),
            ),
          ),
          const SizedBox(height: 24),
