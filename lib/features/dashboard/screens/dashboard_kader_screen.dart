...
class _DashboardKaderScreenState extends State<DashboardKaderScreen> {
    ...
    final confirm = await showDialog<bool>(
      ...
      builder: (_) => AlertDialog(
        ...
        title: Row(children: [
          Container(
            width: 44, height: 44,
            decoration: BoxDecoration(color: _primaryDark, borderRadius: BorderRadius.circular(12)),
            child: Center(child: Text(_getInisial(namaPosyandu),
                style: const TextStyle(color: Colors.white, fontWeight: FontWeight.w800, fontSize: 16))),
          ),
          const SizedBox(width: 12),
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            const Text('Kader Posyandu', style: TextStyle(fontSize: 16, fontWeight: FontWeight.w700)),
            Text(namaPosyandu, style: const TextStyle(fontSize: 13, color: _textGrey)),
          ])),
        ]),
                ...
                if (provider.isLoading)
                  ...
                  SliverFillRemaining(
                    child: Center(child: Column(mainAxisSize: MainAxisSize.min, children: [
                      ...
                      ElevatedButton(onPressed: provider.loadKader, child: const Text('Coba Lagi')),
                    ])),
                  )
                else
                  SliverPadding(
                    ...
                    sliver: SliverList(
                      delegate: SliverChildListDelegate([
                        ...
                        _buildLayananUtama(),
                        const SizedBox(height: 24),
                        _buildAktivitasTerbaru(provider.kaderData?.aktivitasTerbaru ?? []),
                      ]),
                    ),
                  ),
              ],
              ...
              child: Row(children: [
                ...
                GestureDetector(
                  ...
                  child: Container(
                    width: 40, height: 40,
                    decoration: BoxDecoration(color: _primaryDark, borderRadius: BorderRadius.circular(12)),
                    child: Center(child: Text(inisial,
    ...
    return Container(
      ...
      child: Row(children: [
        Container(
          ...
          child: const Icon(Icons.child_care_rounded, color: _primary, size: 28),
        ),
        ...
        Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          Row(crossAxisAlignment: CrossAxisAlignment.end, children: [
            Text('${data?.totalBalita ?? 0}',
                style: const TextStyle(fontSize: 36, fontWeight: FontWeight.w800, color: _textDark)),
            const SizedBox(width: 6),
            const Padding(
              padding: EdgeInsets.only(bottom: 4),
              child: Text('Balita', style: TextStyle(fontSize: 14, color: _textGrey)),
            ),
          ]),
        ]),
      ]),
    ...
    return Container(
      ...
      child: Row(children: [
        ...
        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          const SizedBox(height: 4),
          Text(
            jadwal != null ? _formatTanggal(jadwal.tglKegiatan) : 'Belum ada jadwal',
            style: const TextStyle(fontSize: 16, fontWeight: FontWeight.w700, color: Colors.white),
          ),
          if (jadwal != null)
            Text(jadwal.lokasi, style: const TextStyle(fontSize: 12, color: Colors.white70)),
        ])),
        if (jadwal != null)
          Container(
  ...
  Widget _buildLayananUtama() {
    final menus = [
      _MenuData(icon: Icons.person_add_outlined, label: 'Input Data\nBalita',
          onTap: () => context.push('/anak/tambah')),
      _MenuData(icon: Icons.monitor_heart_outlined, label: 'Catat\nPemeriksaan',
          onTap: () => context.push('/pemeriksaan/catat')),
      _MenuData(icon: Icons.calendar_month_outlined, label: 'Kelola\nJadwal',
    ...
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      if (list.isEmpty)
        Container(
          ...
          child: const Center(child: Text('Belum ada aktivitas', style: TextStyle(color: _textGrey))),
        )
      else
    ...
    return Container(
      ...
      child: Row(children: [
        ...
        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          Text('${_timeAgo(a.tglPeriksa)} • Berat: ${a.beratBadan ?? '-'} kg',
              style: const TextStyle(fontSize: 12, color: _textGrey)),
        ])),
        Container(
          padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 4),
          decoration: BoxDecoration(
            color: isNormal ? const Color(0xFFD1E7DD) : const Color(0xFFFFF3CD),
            borderRadius: BorderRadius.circular(8),
          ),
          child: Text(
            isNormal ? 'NORMAL' : a.statusValidasi.toUpperCase(),
            style: TextStyle(fontSize: 11, fontWeight: FontWeight.w700,
                color: isNormal ? const Color(0xFF198754) : const Color(0xFF856404)),
          ),
        ),
      ]),
    );
  ...
  Widget _buildDataAnakTab() => const DataAnakScreen();
