...
class _JadwalScreenState extends State<JadwalScreen> {
  ...
  bool _hasJadwal(DateTime day, List<JadwalModel> list) {
    return list.any((j) {
      ...
      return jdt.year == day.year && jdt.month == day.month && jdt.day == day.day;
    });
  }
    ...
    if (confirm == true && mounted) {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(
          content: Text(success ? 'Jadwal berhasil dihapus.' : provider.errorMessage ?? 'Gagal menghapus.'),
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: Row(children: [
          ...
          Text(
            isKader ? 'Jadwal Kegiatan' : 'Jadwal Posyandu',
                    ...
                    SliverFillRemaining(
                      child: Center(
                        child: Column(
                          ...
                          children: [
                            const Icon(Icons.calendar_month_outlined, size: 48, color: Color(0xFF94A3B8)),
                            const SizedBox(height: 12),
                            const Text('Belum ada jadwal mendatang',
                                style: TextStyle(fontSize: 15, fontWeight: FontWeight.w600, color: _textDark)),
                            const SizedBox(height: 8),
                            Text(
                              isKader ? 'Tap + untuk membuat jadwal baru' : 'Jadwal belum tersedia',
                      ...
                      sliver: SliverList(
                        delegate: SliverChildBuilderDelegate(
                          (context, index) => Padding(
                            ...
                            child: _buildJadwalCard(provider.upcoming[index], isKader),
                          ),
    ...
    return Container(
      ...
      child: Column(children: [
        Row(
          mainAxisAlignment: MainAxisAlignment.spaceBetween,
          children: [
            Text(monthName, style: const TextStyle(fontSize: 16, fontWeight: FontWeight.w700, color: _textDark)),
            Row(children: [
              GestureDetector(
        ...
        Row(children: dayNames.map((d) => Expanded(
            child: Center(child: Text(d, style: const TextStyle(fontSize: 12, fontWeight: FontWeight.w600, color: _textGrey)))
        )).toList()),
        const SizedBox(height: 8),
        GridView.builder(
          shrinkWrap: true,
          ...
          itemBuilder: (context, index) {
            ...
            final isSelected = _selectedDay != null && day.year == _selectedDay!.year &&
                day.month == _selectedDay!.month && day.day == _selectedDay!.day;
            final hasJadwal = _hasJadwal(day, jadwalList);
            return GestureDetector(
              ...
              child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
                Container(
                  width: 32, height: 32,
                  decoration: BoxDecoration(
                    color: isSelected ? _primary : isToday ? const Color(0xFFEAF2FF) : Colors.transparent,
                    shape: BoxShape.circle,
                  ),
                  child: Center(child: Text('${day.day}', style: TextStyle(
                    fontSize: 13,
                    fontWeight: isToday || isSelected ? FontWeight.w700 : FontWeight.w400,
                    color: isSelected ? Colors.white : isToday ? _primary : _textDark,
                  ))),
                ),
                if (hasJadwal) Container(
                  width: 5, height: 5,
                  margin: const EdgeInsets.only(top: 2),
                  decoration: BoxDecoration(
                    color: isSelected ? Colors.white : _primary, shape: BoxShape.circle,
                  ),
                ),
              ]),
            );
  ...
  Widget _buildJadwalCard(JadwalModel jadwal, bool isKader) {
    final dt     = jadwal.tglDateTime;
    final tglStr = DateFormat('d MMMM yyyy', 'id_ID').format(dt);
      ...
      child: Row(
        ...
        children: [
          Container(
            ...
            child: const Icon(Icons.emoji_emotions_outlined, color: _primary, size: 22),
          ),
          ...
          Expanded(
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Row(children: [
                ...
                // Tombol edit & hapus hanya untuk Kader
                if (isKader) ...[
                  GestureDetector(
                    onTap: () => _showFormJadwal(jadwal: jadwal),
                    child: const Icon(Icons.edit_outlined, color: _textGrey, size: 18),
                  ),
              ...
              Row(children: [
                const Icon(Icons.calendar_today_outlined, size: 13, color: _textGrey),
                const SizedBox(width: 4),
                Text(tglStr, style: const TextStyle(fontSize: 12, color: _textGrey)),
              ]),
              if (jadwal.agenda != null) ...[
                ...
                Container(
                  ...
                  decoration: BoxDecoration(color: const Color(0xFFF1F5F9), borderRadius: BorderRadius.circular(8)),
                  child: Row(children: [
                    const Icon(Icons.notes_outlined, size: 13, color: _textGrey),
                    const SizedBox(width: 4),
                    Expanded(child: Text(jadwal.agenda!,
...
class _JadwalFormSheetState extends State<_JadwalFormSheet> {
  ...
  String get _tglDisplay => _tglKegiatan == null
      ? 'Pilih Tanggal'
      : DateFormat('d MMMM yyyy', 'id_ID').format(_tglKegiatan!);

  String get _tglFormatted => _tglKegiatan == null ? ''
      : '${_tglKegiatan!.year}-${_tglKegiatan!.month.toString().padLeft(2,'0')}-${_tglKegiatan!.day.toString().padLeft(2,'0')}';
  ...
  Future<void> _pickDate() async {
    final picked = await showDatePicker(
      ...
      builder: (context, child) => Theme(
        data: Theme.of(context).copyWith(colorScheme: const ColorScheme.light(primary: _primary)),
        child: child!,
      ),
    );
  ...
  Future<void> _submit() async {
    if (_judulCtrl.text.trim().isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
        content: Text('Judul kegiatan wajib diisi'), backgroundColor: Color(0xFFDC3545),
        behavior: SnackBarBehavior.floating,
      ));
      return;
    }
    if (_tglKegiatan == null) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
        content: Text('Pilih tanggal kegiatan'), backgroundColor: Color(0xFFDC3545),
        behavior: SnackBarBehavior.floating,
      ));
      return;
    }
    ...
    if (isEdit) {
      success = await provider.update(
        idJadwal: widget.jadwal!.idJadwal, tglKegiatan: _tglFormatted,
        lokasi: _judulCtrl.text.trim(),
        agenda: _deskripsiCtrl.text.trim().isEmpty ? null : _deskripsiCtrl.text.trim(),
      );
    } else {
      success = await provider.buat(
        tglKegiatan: _tglFormatted, lokasi: _judulCtrl.text.trim(),
        agenda: _deskripsiCtrl.text.trim().isEmpty ? null : _deskripsiCtrl.text.trim(),
      );
    }
    if (mounted) {
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(
        content: Text(success ? provider.successMessage ?? 'Berhasil!' : provider.errorMessage ?? 'Gagal menyimpan.'),
        backgroundColor: success ? const Color(0xFF198754) : const Color(0xFFDC3545),
        behavior: SnackBarBehavior.floating,
        shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(10)),
        margin: const EdgeInsets.all(16),
      ));
  ...
  @override
  Widget build(BuildContext context) {
    return Consumer<JadwalProvider>(builder: (context, provider, _) {
      return Container(
        ...
        child: SingleChildScrollView(
          child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            const Text('Buat agenda kegiatan Posyandu mendatang.',
            ...
            GestureDetector(
              ...
              child: Container(
                ...
                decoration: BoxDecoration(
                  color: _inputBg, borderRadius: BorderRadius.circular(16),
                  border: Border.all(color: _tglKegiatan != null ? _primary : _border,
                      width: _tglKegiatan != null ? 1.5 : 1),
                ),
                child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
                  Text(_tglDisplay, style: TextStyle(fontSize: 14,
                      color: _tglKegiatan != null ? _textDark : const Color(0xFFCBD5E1))),
                  Icon(Icons.calendar_month_outlined,
                      color: _tglKegiatan != null ? _primary : const Color(0xFF94A3B8), size: 20),
                ]),
              ),
            ),
            const SizedBox(height: 16),
            ...
            const SizedBox(height: 8),
            TextField(
              controller: _deskripsiCtrl, maxLines: 3,
              decoration: InputDecoration(
                hintText: 'Tambahkan catatan singkat mengenai kegiatan...',
                hintStyle: const TextStyle(color: Color(0xFFCBD5E1)),
                filled: true, fillColor: _inputBg, contentPadding: const EdgeInsets.all(14),
                border: OutlineInputBorder(borderRadius: BorderRadius.circular(16), borderSide: BorderSide.none),
                focusedBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(16),
                    borderSide: const BorderSide(color: _primary, width: 1.5)),
              ),
            ...
            Center(child: TextButton(
              ...
              child: const Text('Batal', style: TextStyle(fontSize: 14, color: _textGrey)),
            )),
          ]),
        ),
      );
  ...
  Widget _buildLabel(String text) => Text(text,
      style: const TextStyle(fontSize: 11, fontWeight: FontWeight.w700, color: _textGrey, letterSpacing: 1));
}